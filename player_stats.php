...
if (!empty($teamIDs)) {
    if (isset($teams) && $teams instanceof MongoDB\Collection) {
        ...
        foreach ($teamIDs as $id) {
            if (is_string($id)) { $teamDisplayNames[$id] = $latestTeamNameMap[$id]['name'] ?? $id; }
        }
    } else {
        foreach ($teamIDs as $id) { if (is_string($id)) { $teamDisplayNames[$id] = $id; } }
    }
...
if (!empty($playerMatchStage)) { $aggregationPipeline[] = ['$match' => $playerMatchStage]; }
$aggregationPipeline[] = ['$unwind' => '$career_teams'];
$seasonTeamMatchStage = [];
if (!empty($filterSeasons)) { $seasonTeamMatchStage['career_teams.year'] = ['$in' => $filterSeasons]; }
if ($filterTeam) { $seasonTeamMatchStage['career_teams.tmID'] = $filterTeam; }
if (!empty($seasonTeamMatchStage)) { $aggregationPipeline[] = ['$match' => $seasonTeamMatchStage]; }
...
<head>
    ...
    <style>
        body { font-family: 'Montserrat', sans-serif; background-color: #0A0A14; color: #E0E0E0; }
        .font-teko { font-family: 'Teko', sans-serif; }
        .font-rajdhani { font-family: 'Rajdhani', sans-serif; }
        ::-webkit-scrollbar { width: 8px; height: 8px; }
        ::-webkit-scrollbar-track { background: #111827; }
        ::-webkit-scrollbar-thumb { background: #374151; border-radius: 10px; }
        .content-container { background-color: rgba(23, 23, 38, 0.7); border: 1px solid rgba(55, 65, 81, 0.4); border-radius: 0.75rem; padding: 1.5rem; backdrop-filter: blur(8px); box-shadow: 0 8px 32px rgba(0, 0, 0, 0.3); }
        select, input[type="text"] { background-color: rgba(17, 24, 39, 0.8); border: 1px solid #374151; color: #E0E0E0; border-radius: 0.375rem; }
        select:focus, input:focus { outline: 2px solid transparent; outline-offset: 2px; border-color: #3B82F6; box-shadow: 0 0 0 3px rgba(59, 130, 246, 0.3); }
        ...
        .btn-primary { background-color: #1D4ED8; color: white; transition: all 0.2s; font-weight: 600; padding: 0.625rem 1.25rem; border-radius: 0.375rem; }
        .btn-primary:hover { background-color: #1E40AF; transform: translateY(-2px); }
        .pagination a { padding: 0.5rem 0.875rem; border: 1px solid #374151; background-color: rgba(17, 24, 39, 0.5); color: #D1D5DB; font-size: 0.875rem; font-weight: 500; transition: all 0.15s; border-radius: 0.375rem; }
        .pagination a:hover { background-color: #374151; border-color: #4b5563; }
        .pagination a.active { background-color: #1D4ED8; color: white; border-color: #1D4ED8; font-weight: 600; }
        .pagination a.disabled { color: #6b7280; cursor: not-allowed; background-color: #1f2937; border-color: #374151; }
        .player-tags-container { display: flex; flex-wrap: wrap; align-items: center; gap: 0.5rem; padding: 0.25rem 0.5rem; border: 1px solid #374151; border-radius: 0.375rem; background-color: rgba(17, 24, 39, 0.8); min-height: 42px; cursor: text; transition: border-color 0.2s, box-shadow 0.2s; }
        .player-tags-container:focus-within { border-color: #3B82F6; box-shadow: 0 0 0 3px rgba(59, 130, 246, 0.3); }
        .player-tag { display: inline-flex; align-items: center; background-color: #2563EB; color: white; padding: 0.25rem 0.75rem; border-radius: 9999px; font-size: 0.8rem; font-weight: 500; }
        .remove-tag-btn { margin-left: 0.5rem; background: none; border: none; color: #bfdbfe; cursor: pointer; opacity: 0.7; }
        #newPlayerName { flex-grow: 1; background: transparent; border: none; outline: none; box-shadow: none; padding: 0.25rem; min-width: 120px; }
        .player-radar-checkbox { cursor: pointer; width: 1.15rem; height: 1.15rem; accent-color: #3B82F6; }
        ...
        .info-tooltip-container .info-tooltip-text {
            visibility: hidden; width: 220px; background-color: #1f2937; color: #d1d5db;
                ...
                <table class="min-w-full table-fixed text-sm">
                    <thead class="sticky top-0 bg-gray-900/70 backdrop-blur-sm z-10">
                        <tr class="text-left text-xs text-gray-400 uppercase tracking-wider">
                            ...
                            <th class="px-4 py-3 font-semibold w-2/12">Team</th><th class="px-4 py-3 font-semibold w-[6%] text-center">Pos</th>
                            <th class="px-4 py-3 font-semibold w-[7%] text-center">Season</th><th class="px-4 py-3 font-semibold w-[6%] text-right">GP</th>
                            <th class="px-4 py-3 font-semibold w-[7%] text-right">Pts</th><th class="px-4 py-3 font-semibold w-[7%] text-right">Ast</th>
                            <th class="px-4 py-3 font-semibold w-[7%] text-right">Reb</th><th class="px-4 py-3 font-semibold w-[6%] text-right">Stl</th>
                            <th class="px-4 py-3 font-semibold w-[6%] text-right">Blk</th><th class="px-4 py-3 font-semibold w-[6%] text-right">TO</th>
                            <th class="px-4 py-3 font-semibold w-[8%] text-right">FG%</th><th class="px-4 py-3 font-semibold w-[10%] text-center">Details</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-gray-800">
                        ...
                        <?php endforeach; else: ?>
                            <tr><td colspan="14" class="text-center p-10 text-gray-500"><div class="flex flex-col items-center"><i class="fas fa-ghost fa-3x text-gray-700 mb-4"></i><p class="font-semibold text-gray-300 text-base">No Player Data Found</p><p class="text-sm">Try adjusting your filters.</p></div></td></tr>
                        <?php endif; ?>
                    </tbody>
                </table>
            </div>
            ...
            <?php if ($totalPages > 1): ?>
                <div class="pagination px-5 py-4 border-t border-gray-700 flex flex-col sm:flex-row justify-between items-center space-y-3 sm:space-y-0">
                    ...
                    <div class="flex items-center space-x-1.5">
                        <?php $queryParams = $_GET; unset($queryParams['page']); $baseUrl = '?' . http_build_query($queryParams) . (empty($queryParams) ? '' : '&'); ?>
                        ...
                        <?php
                            ...
                            if ($startPage > 1) { echo '<a href="' . $baseUrl . 'page=1">1</a>'; if ($startPage > 2) echo '<span class="px-2 py-1.5 text-gray-500 text-xs">...</span>'; }
                            for ($i = $startPage; $i <= $endPage; $i++): ?><a href="<?= $baseUrl . 'page=' . $i ?>" class="<?= ($i == $currentPage) ? 'active' : '' ?>"><?= $i ?></a><?php endfor;
                            if ($endPage < $totalPages) { if ($endPage < $totalPages - 1) echo '<span class="px-2 py-1.5 text-gray-500 text-xs">...</span>'; echo '<a href="' . $baseUrl . 'page=' . $totalPages . '">' . $totalPages . '</a>'; }
                        ?>
                ...
                <?php if (count($data) > 0): ?>
                    <div class="content-container min-h-[400px] md:min-h-[450px] flex flex-col">
                        <div class="flex flex-col sm:flex-row justify-between items-center mb-4 gap-2">
                            ...
                            <div class="flex items-center space-x-2"><label for="barChartSort" class="text-xs text-gray-400">Sort by:</label><select id="barChartSort" class="text-xs py-1 pl-2 pr-8 rounded border-gray-600 bg-gray-800 text-gray-200 focus:border-blue-500 focus:ring-blue-500"><option value="default">Default Order</option><option value="points">Points (High to Low)</option><option value="assists">Assists (High to Low)</option><option value="rebounds">Rebounds (High to Low)</option></select></div>
                        </div>
                    ...
                    <div class="content-container min-h-[400px] md:min-h-[450px] flex flex-col">
                        ...
                        <div class="flex-grow relative">
                             ...
                             <div id="radar-placeholder" class="absolute inset-0 flex flex-col items-center justify-center text-center text-gray-500 p-4"><i class="fas fa-bullseye fa-3x text-gray-600 mb-4"></i><p class="font-semibold text-gray-400">Select 2 to 5 Players</p><p class="text-xs">Use the checkboxes in the table above to compare.</p></div>
                        </div>
                    </div>
                <?php else: ?>
                    <div class="lg:col-span-2 content-container min-h-[400px]"><div class="flex flex-col items-center justify-center h-full text-gray-500 p-6 text-center"><i class="fas fa-chart-pie fa-3x text-gray-700 mb-4"></i><p class="text-sm font-medium">No data to display charts.</p><p class="text-xs">Adjust the filters to see visual insights.</p></div></div>
                <?php endif; ?>
            </div>
        </section>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const RADAR_COLORS = [
        { fill: 'rgba(59, 130, 246, 0.4)', stroke: 'rgba(96, 165, 250, 1)' }, { fill: 'rgba(239, 68, 68, 0.4)', stroke: 'rgba(248, 113, 113, 1)' },
        { fill: 'rgba(16, 185, 129, 0.4)', stroke: 'rgba(52, 211, 153, 1)' }, { fill: 'rgba(245, 158, 11, 0.4)', stroke: 'rgba(251, 191, 36, 1)' },
        { fill: 'rgba(168, 85, 247, 0.4)', stroke: 'rgba(192, 132, 252, 1)' },
    ];
    ...
    if (playerTagsContainer) {
        ...
        function renderPlayerTags() {
            ...
            currentPlayers.forEach((name, index) => {
                ...
                hiddenInput.type = 'hidden'; hiddenInput.name = 'players[]'; hiddenInput.value = name;
                hiddenInputsContainer.appendChild(hiddenInput);
            });
        }
        newPlayerInput.addEventListener('keydown', function(event) {
            if (event.key === 'Enter') {
                event.preventDefault(); const newName = this.value.trim();
                if (newName && !currentPlayers.some(p => p.toLowerCase() === newName.toLowerCase())) { currentPlayers.push(newName); this.value = ''; renderPlayerTags(); }
            }
        });
        playerTagsContainer.addEventListener('click', function(event) {
            if (event.target.classList.contains('remove-tag-btn')) {
                const indexToRemove = parseInt(event.target.dataset.index, 10);
                currentPlayers.splice(indexToRemove, 1); renderPlayerTags();
            } else if (event.target === playerTagsContainer) { newPlayerInput.focus(); }
        });
        renderPlayerTags();
    }
    ...
    function updateBarChart(sortBy = 'default') {
        ...
        if (sortBy !== 'default') { chartData.sort((a, b) => (b[sortBy] || 0) - (a[sortBy] || 0)); }
        const labels = chartData.map(p => `${p.playerName} (${p.pos || 'N/A'})`);
        const chartConfigData = {
            labels: labels,
            datasets: [
                { label: 'Points', data: chartData.map(p => p.points), backgroundColor: 'rgba(59, 130, 246, 0.7)', borderColor: 'rgba(59, 130, 246, 1)', borderWidth: 1 },
                { label: 'Assists', data: chartData.map(p => p.assists), backgroundColor: 'rgba(16, 185, 129, 0.7)', borderColor: 'rgba(16, 185, 129, 1)', borderWidth: 1 },
                { label: 'Rebounds', data: chartData.map(p => p.rebounds), backgroundColor: 'rgba(245, 158, 11, 0.7)', borderColor: 'rgba(245, 158, 11, 1)', borderWidth: 1 }
            ]
        };
        if (barChartInstance) { barChartInstance.data = chartConfigData; barChartInstance.update(); } 
        else {
            const ctx = document.getElementById('barChartPlayers')?.getContext('2d'); if(!ctx) return;
            barChartInstance = new Chart(ctx, { type: 'bar', data: chartConfigData, options: { responsive: true, maintainAspectRatio: false, scales: { y: { beginAtZero: true, grid: { color: 'rgba(255, 255, 255, 0.1)' }, ticks: { color: '#9CA3AF' } }, x: { grid: { display: false }, ticks: { color: '#9CA3AF' } } }, plugins: { legend: { position: 'top', labels: { color: '#E0E0E0', usePointStyle: true, boxWidth: 8 } }, tooltip: { backgroundColor: '#1F2937', titleColor: '#E5E7EB', bodyColor: '#D1D5DB', boxPadding: 4, callbacks: { label: function(context) { let label = context.dataset.label || ''; if (label) { label += ': '; } if (context.parsed.y !== null) { label += context.parsed.y; } return label; } } } } } });
        }
    }
    if (barChartSortSelect) { barChartSortSelect.addEventListener('change', function() { updateBarChart(this.value); }); }
    ...
    function updateRadarChart() {
        ...
        let playersWithScores = selectedPlayers.map(p => {
            const perGameStats = {};
            STAT_KEYS.forEach(key => { perGameStats[key] = p.GP > 0 ? parseFloat((p[key] / p.GP).toFixed(1)) : 0; });
            return { ...p, perGameStats };
        });

        const maxStats = {};
        STAT_KEYS.forEach(key => { maxStats[key] = Math.max(...playersWithScores.map(p => p.perGameStats[key]), 0.1); });
        ...
        playersWithScores = playersWithScores.map(p => {
            ...
            return { ...p, normalizedData, dominanceScore };
        });
        ...
        const datasets = playersWithScores.map((player, index) => {
            ...
            return {
                ...
                backgroundColor: color.fill, borderColor: color.stroke, pointBackgroundColor: color.stroke,
                pointBorderColor: '#fff', pointHoverBackgroundColor: '#fff', pointHoverBorderColor: color.stroke, borderWidth: 2.5
            };
        });
        ...
        if (radarChartInstance) {
            ...
            radarChartInstance = new Chart(ctx, { type: 'radar', data: { labels: STAT_LABELS, datasets: datasets }, options: { responsive: true, maintainAspectRatio: false, elements: { line: { tension: 0.1, fill: true } }, scales: { r: { angleLines: { color: 'rgba(255, 255, 255, 0.15)' }, grid: { color: 'rgba(255, 255, 255, 0.15)' }, pointLabels: { color: '#E0E0E0', font: { size: 13, family: "'Rajdhani', sans-serif", weight: '700' } }, ticks: { display: false, stepSize: 25 }, suggestedMin: 0, suggestedMax: 100 } }, plugins: { legend: { position: 'bottom', labels: { color: '#E0E0E0', usePointStyle: true, boxWidth: 8, padding: 20, font: { size: 12 } } }, tooltip: { backgroundColor: '#111827', borderColor: 'rgba(255,255,255,0.2)', borderWidth: 1, titleColor: '#E5E7EB', bodyColor: '#D1D5DB', boxPadding: 8, callbacks: { label: function(context) { const player = playersWithScores[context.datasetIndex]; const statKey = STAT_KEYS[context.dataIndex]; const originalValue = player.perGameStats[statKey]; return `${player.playerName}: ${originalValue.toFixed(1)} ${statKey.toUpperCase()} per game`; } } } } } });
        }
    }
    ...
    document.querySelectorAll('.player-radar-checkbox').forEach(checkbox => {
        checkbox.addEventListener('change', function() {
            ...
            if (this.checked) { if (selectedPlayerIndices.size < 5) { selectedPlayerIndices.add(playerIndex); } else { this.checked = false; alert('You can compare a maximum of 5 players at a time.'); return; } } else { selectedPlayerIndices.delete(playerIndex); }
            updateRadarChart();
        });
    });

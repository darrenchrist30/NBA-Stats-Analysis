...
// --- PENGAMBILAN DATA UNIK UNTUK DROPDOWN (TIDAK BERUBAH) ---
...
$positions = array_filter($positions, fn($v) => !is_null($v) && $v !== '');
sort($positions);

// --- PIPELINE AGREGRASI UTAMA (TIDAK BERUBAH) ---
$aggregationPipeline = [];
...
if (!empty($filterPlayerNames)) {
    ...
    foreach ($filterPlayerNames as $name) {
        $orConditions[] = ['$or' => [['firstName' => new MongoDB\BSON\Regex(preg_quote($name, '/'), 'i')], ['lastName' => new MongoDB\BSON\Regex(preg_quote($name, '/'), 'i')], ['fullName' => new MongoDB\BSON\Regex(preg_quote($name, '/'), 'i')], ['useFirst' => new MongoDB\BSON\Regex(preg_quote($name, '/'), 'i')]]];
    }
...
// Hitung total entri SEBELUM paginasi
$countPipeline = array_merge($aggregationPipeline, [['$count' => 'total']]);
$countResult = $players_collection->aggregate($countPipeline)->toArray();
$totalPlayerSeasonEntries = $countResult[0]['total'] ?? 0;
...
// --- MODIFIKASI: QUERY KHUSUS UNTUK BAR CHART (TIDAK TERPENGARUH PAGINASI) ---
$barChartPipeline = array_merge($aggregationPipeline, [['$sort' => ['career_teams.points' => -1]], ['$limit' => 20]]);
$barChartDataResult = $players_collection->aggregate($barChartPipeline)->toArray();
$barChartData = [];
foreach ($barChartDataResult as $doc) {
    $playerSeasonData = $doc['career_teams'];
    $barChartData[] = [
        'playerName' => trim(($doc['useFirst'] ?? ($doc['firstName'] ?? '')) . ' ' . ($doc['lastName'] ?? 'Unknown')),
        'pos' => $doc['pos'] ?? 'N/A',
        'year' => $playerSeasonData['year'] ?? '-',
...
if ($radarEnabled) {
    ...
    if (!empty($maxStatsResult)) {
        $maxStatsPerSeason = $maxStatsResult[0];
    }
}
...
foreach ($cursor as $doc) {
    // ... Pengisian data tabel tidak berubah ...
    $playerSeasonData = $doc['career_teams'];
    $playerEntry = ['playerID' => $doc['playerID'] ?? null, 'playerName' => trim(($doc['useFirst'] ?? ($doc['firstName'] ?? '')) . ' ' . ($doc['lastName'] ?? 'Unknown')), 'pos' => $doc['pos'] ?? '-', 'year' => $playerSeasonData['year'] ?? null, 'tmID' => $playerSeasonData['tmID'] ?? null, 'GP' => $playerSeasonData['GP'] ?? 0, 'points' => $playerSeasonData['points'] ?? 0, 'assists' => $playerSeasonData['assists'] ?? 0, 'rebounds' => $playerSeasonData['rebounds'] ?? 0, 'steals' => $playerSeasonData['steals'] ?? 0, 'blocks' => $playerSeasonData['blocks'] ?? 0, 'turnovers' => $playerSeasonData['turnovers'] ?? 0, 'fgMade' => $playerSeasonData['fgMade'] ?? 0, 'fgAttempted' => $playerSeasonData['fgAttempted'] ?? 0, 'normalized_stats' => null];
    if ($radarEnabled && !empty($maxStatsPerSeason)) {
        $gp = $playerEntry['GP'];
        $ppg = $gp > 0 ? $playerEntry['points'] / $gp : 0;
        $rpg = $gp > 0 ? $playerEntry['rebounds'] / $gp : 0;
        $apg = $gp > 0 ? $playerEntry['assists'] / $gp : 0;
        $spg = $gp > 0 ? $playerEntry['steals'] / $gp : 0;
        $bpg = $gp > 0 ? $playerEntry['blocks'] / $gp : 0;
        $playerEntry['normalized_stats'] = ['scoring' => ($maxStatsPerSeason['max_points'] > 0) ? round(($ppg / $maxStatsPerSeason['max_points']) * 100) : 0, 'rebounding' => ($maxStatsPerSeason['max_rebounds'] > 0) ? round(($rpg / $maxStatsPerSeason['max_rebounds']) * 100) : 0, 'playmaking' => ($maxStatsPerSeason['max_assists'] > 0) ? round(($apg / $maxStatsPerSeason['max_assists']) * 100) : 0, 'steals' => ($maxStatsPerSeason['max_steals'] > 0) ? round(($spg / $maxStatsPerSeason['max_steals']) * 100) : 0, 'defense' => ($maxStatsPerSeason['max_blocks'] > 0) ? round(($bpg / $maxStatsPerSeason['max_blocks']) * 100) : 0,];
    }
    $data[] = $playerEntry;
}
...
<head>
    ...
    <style>
        ...
        .player-radar-checkbox:disabled {
            cursor: not-allowed;
            opacity: 0.5;
        }
...
<body class="antialiased">
    <div class="max-w-screen-2xl mx-auto p-4 md:p-6 lg:p-8">
        <!-- Header, Filter, KPI Cards, Tabel, Paginasi (HTML tidak berubah) -->
        ...
        <div class="content-container mb-8 sticky top-4 z-20">
            <form method="GET" id="playerFilterForm" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-x-4 gap-y-4 items-end">
                <div><label for="season-btn" class="block text-xs font-medium text-gray-400 mb-1.5">Season(s)</label>
                    <div class="relative" id="season-dropdown-container"><button type="button" id="season-btn" class="w-full text-sm py-2.5 px-3 text-left flex justify-between items-center bg-gray-800/80 border border-gray-700 rounded-md"> <span class="truncate">Select Seasons</span> <i class="fas fa-chevron-down fa-xs text-gray-400 transition-transform duration-200"></i> </button>
                        <div id="season-checklist" class="absolute hidden w-full mt-1 bg-gray-800 border border-gray-700 rounded-md shadow-lg z-30 max-h-60 overflow-y-auto"><?php foreach ($seasons as $season): ?> <label class="flex items-center w-full px-3 py-2 text-sm text-gray-200 hover:bg-gray-700 cursor-pointer"> <input type="checkbox" name="seasons[]" value="<?= htmlspecialchars($season) ?>" class="h-4 w-4 rounded border-gray-500 bg-gray-700 text-blue-500 focus:ring-blue-500/50 mr-3 season-checkbox" <?= in_array($season, $filterSeasons) ? 'checked' : '' ?>> <?= htmlspecialchars($season) ?> </label> <?php endforeach; ?></div>
                    </div>
                </div>
                ...
                <div><label for="newPlayerName" class="block text-xs font-medium text-gray-400 mb-1.5">Player Name(s)</label>
                    <div id="player-tags-input-container" class="player-tags-container">
                        <div id="hidden-player-inputs"></div><input type="text" id="newPlayerName" class="text-sm" placeholder="Type name & press Enter...">
                    </div>
                </div><button type="submit" class="btn btn-primary w-full flex items-center justify-center h-[42px] uppercase tracking-wider text-sm"> <i class="fas fa-filter mr-2 fa-sm"></i> Apply </button>
            </form>
        </div>
        ...
        <div class="content-container !p-0 overflow-hidden mb-8">
            <div class="px-5 py-4 border-b border-gray-700 flex flex-col sm:flex-row justify-between items-center gap-y-3">
                ...
                <div class="flex items-center gap-x-4"><?php if ($totalPlayerSeasonEntries > 0): $startEntry = ($currentPage - 1) * $itemsPerPage + 1;
                                                            $endEntry = min($currentPage * $itemsPerPage, $totalPlayerSeasonEntries); ?><span class="text-xs font-medium text-gray-400"> Showing <?= number_format($startEntry) ?>-<?= number_format($endEntry) ?> of <?= number_format($totalPlayerSeasonEntries) ?> entries </span><?php endif; ?><div class="flex items-center space-x-2"><label for="itemsPerPageSelect" class="text-xs font-medium text-gray-400">Show:</label><select id="itemsPerPageSelect" name="limit" class="text-xs py-1 pl-2 pr-7 rounded border-gray-600 bg-gray-800 text-gray-200 focus:border-blue-500 focus:ring-blue-500/50"><?php foreach ($validLimits as $limitOption): ?><option value="<?= $limitOption ?>" <?= ($itemsPerPage == $limitOption) ? 'selected' : '' ?>><?= $limitOption ?></option><?php endforeach; ?></select></div>
                </div>
            </div>
            <div class="overflow-x-auto">
                <table class="min-w-full table-fixed text-sm">
                    ...
                    <tbody class="divide-y divide-gray-800"><?php if (count($data) > 0): foreach ($data as $index => $row): ?><?php $fgPercent = ($row['fgAttempted'] > 0) ? round(($row['fgMade'] / $row['fgAttempted']) * 100, 1) : 0;
                                                                                                                                $teamNameForDisplay = $teamDisplayNames[$row['tmID']] ?? $row['tmID']; ?><tr class="hover:bg-gray-800/60 transition-colors duration-100 group">
                            <td class="px-4 py-2.5 text-center"><input type="checkbox" class="player-radar-checkbox" data-player-index="<?= $index ?>" <?= !$radarEnabled ? 'disabled' : '' ?>></td>
            ...
            <?php if ($totalPages > 1): ?><div class="pagination px-5 py-4 border-t border-gray-700 flex flex-col sm:flex-row justify-between items-center space-y-3 sm:space-y-0">
                ...
                <?php if (count($data) > 0): ?>
                    ...
                    <div class="content-container min-h-[400px] md:min-h-[450px] flex flex-col">
                        <div class="flex items-center gap-2 mb-4 text-center sm:text-left">
                            ...
                            <div class="info-tooltip-container"><i class="fas fa-info-circle text-gray-400 hover:text-gray-200 cursor-pointer"></i><span class="info-tooltip-text">The Dominance Score (%) is an average of the player's normalized stats, showing their overall performance relative to all players in the selected season.</span></div>
                        </div>
                        <div class="flex-grow relative">
                            <canvas id="radarChart"></canvas>
                            <div id="radar-placeholder" class="absolute inset-0 flex flex-col items-center justify-center text-center text-gray-500 p-4">
                                <?php if ($radarEnabled): ?>
                                    <i class="fas fa-bullseye fa-3x text-gray-600 mb-4"></i>
                                    <p class="font-semibold text-gray-400">Select 1 to 5 Players</p>
                                    <p class="text-xs">Use the checkboxes in the table above to compare.</p>
                                <?php else: ?>
                                    <i class="fas fa-exclamation-triangle fa-3x text-gray-600 mb-4"></i>
                                    <p class="font-semibold text-gray-400">Radar Chart Disabled</p>
                                    <p class="text-xs">Please select a <b>single season</b> in the filter to enable player comparison.</p>
                                <?php endif; ?>
                            </div>
                        </div>
                    </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            // --- MODIFIKASI: GUNAKAN SUMBER DATA TERPISAH UNTUK BAR CHART ---
            const tableChartDataSource = <?= json_encode($data); ?>;
            const barChartDataSource = <?= json_encode($barChartData); ?>; // Data baru untuk Bar Chart
            ...
            if (seasonBtn) {
                ...
                function updateSeasonButtonText() {
                    const checkedSeasons = Array.from(seasonCheckboxes).filter(cb => cb.checked).map(cb => cb.value);
                ...
                document.addEventListener('click', (e) => {
                    if (seasonDropdownContainer && !seasonDropdownContainer.contains(e.target)) {
            ...
            function updateBarChart(sortBy = 'default') {
                ...
                // Gunakan barChartDataSource yang tidak terpengaruh paginasi
                const chartData = [...barChartDataSource];
                if (sortBy !== 'default') {
                    chartData.sort((a, b) => (b[sortBy] || 0) - (a[sortBy] || 0));
                }
                const labels = chartData.map(p => `${p.playerName} (${p.year})`);
            ...
            // --- RADAR CHART LOGIC (TIDAK BERUBAH) ---
            const radarPlaceholder = document.getElementById('radar-placeholder');
            const radarIsEnabled = <?= json_encode($radarEnabled); ?>;
            const STAT_LABELS = ['Scoring', 'Rebounding', 'Playmaking', 'Steals', 'Defense'];

            function updateRadarChart() {
                if (!document.getElementById('radarChart') || !radarIsEnabled) return;
                const selectedPlayers = Array.from(selectedPlayerIndices).map(index => tableChartDataSource[index]);
                ...
                const playersWithScores = selectedPlayers.map(p => {
                    if (!p.normalized_stats) return {
                        ...p,
                        dominanceScore: 0,
                        normalizedData: [0, 0, 0, 0, 0]
                    };
                    const stats = p.normalized_stats;
                    const normalizedData = [stats.scoring, stats.rebounding, stats.playmaking, stats.steals, stats.defense];
            ...
            if (barChartDataSource.length > 0) {
                updateBarChart();
            }
            if (tableChartDataSource.length > 0 && radarIsEnabled) {
                updateRadarChart();
            }
        });
    </script>

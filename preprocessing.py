...
if not df_teams_annual_raw.empty:
    ...
    # Gunakan 'games' jika ada dan > 0, jika tidak gunakan kalkulasi
    df_teams_annual_raw['games_played'] = df_teams_annual_raw.apply(
        lambda row: row['games'] if pd.notna(row['games']) and row['games'] > 0 else row['total_games_calc'], axis=1
    )
    df_teams_annual_raw['win_pct'] = (df_teams_annual_raw['won'] / df_teams_annual_raw['games_played'] * 100).fillna(0).round(1)

    print("\nContoh data tim tahunan dengan Win % (5 baris pertama):")
    print(df_teams_annual_raw[['year', 'tmID', 'name', 'won', 'lost', 'games_played', 'win_pct', 'playoff']].head())

    # B. Cek nilai unik untuk kolom kategorikal penting
    print("\nPlayoff outcomes (unik):")
    if 'playoff' in df_teams_annual_raw.columns:
        print(df_teams_annual_raw['playoff'].value_counts())
    
    # C. Buat Label Dekade
    df_teams_annual_raw_cleaned = df_teams_annual_raw.dropna(subset=['year'])
    df_teams_annual_raw_cleaned['year'] = df_teams_annual_raw_cleaned['year'].astype(int)
    df_teams_annual_raw_cleaned['decade_label'] = ((df_teams_annual_raw_cleaned['year'] // 10) * 10).astype(str) + "s"
    print("\nJumlah musim tim per dekade:")
    print(df_teams_annual_raw_cleaned['decade_label'].value_counts().sort_index())

else:
    print("DataFrame tim tahunan kosong, pre-processing tim dilewati.")


# --- 5. Contoh Analisis Sederhana dari `df_series_post_raw` ---
if not df_series_post_raw.empty:
    print("\n--- Analisis Sederhana Data Seri Playoff ---")
    
    print(f"Jumlah total seri playoff dalam dataset: {len(df_series_post_raw)}")
    
    # Tim dengan kemenangan seri playoff terbanyak
    if 'tmIDWinner' in df_series_post_raw.columns:
        print("\nTim dengan kemenangan seri playoff terbanyak (Top 5):")
        print(df_series_post_raw['tmIDWinner'].value_counts().head())

    # Skor seri yang paling umum
    if 'W' in df_series_post_raw.columns and 'L' in df_series_post_raw.columns:
        df_series_post_raw['series_score'] = df_series_post_raw['W'].astype(str) + '-' + df_series_post_raw['L'].astype(str)
        print("\nSkor seri playoff yang paling umum (Top 5):")
        print(df_series_post_raw['series_score'].value_counts().head())
else:
    print("DataFrame seri playoff kosong, analisis playoff dilewati.")


# --- Opsional: Simpan DataFrame yang sudah diproses jika perlu ---
# Misalnya, jika Anda ingin menyimpan df_player_seasons:
# if not df_player_seasons.empty:
#     try:
#         df_player_seasons.to_json('player_seasons_processed.json', orient='records', indent=2)
#         print("\nDataFrame musim pemain yang diproses disimpan ke player_seasons_processed.json")
#     except Exception as e:
#         print(f"ERROR saat menyimpan player_seasons_processed.json: {e}")
